...
export function AuthorDisplay({ author, createdAt }: AuthorDisplayProps) {
  const fallbackId = author.id || 'unknown'
  const shortId = fallbackId ? `${fallbackId.slice(0, 6)}…` : 'unknown'
  ...
  return (
    <div className="flex items-center gap-1 text-sm">
      <span className="font-semibold hover:underline truncate max-w-[160px]" title={displayName}>
        {displayName}
      </span>
      {verified && (
